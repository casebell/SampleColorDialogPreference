...
public class FontPreference extends ListPreference 
{
    ...
    public void onClick(DialogInterface dialog, int which)
    {
        ...
        if ( which >=0 && which < m_fontPaths.size() )
        {
            ...
            String selectedFontName = m_fontNames.get(which);
            Editor editor = getSharedPreferences().edit();
            editor.putString( getKey(), selectedFontName );
            editor.putString( selectedFontName, selectedFontPath );
            editor.commit();
            dialog.dismiss();
            callChangeListener(selectedFontName);
        }
    }
